...
class JokeMachineSession(ExportedGObject):
    ...
    def hello_cb(self, sender=None):
        ...
        logging.debug(
            'In hello_cb - Newcomer %s has joined and sent Hello',
            sender)
    ...
    def PumpActivityState(self, state_pickle):
        ...
        if len(state_pickle) == 0:
            logging.debug(
                "JokeMachineSession.ReceiveActivityState()"
                " -> empty state_pickle - creating empty state")
            activity_state = JokeMachineState().test_data()
        else:
            logging.debug(
                "JokeMachineSession.ReceiveActivityState()"
                " -> Unpickling state from remote")
    ...
    def submit_cb(self, jokebook_id, joke_pickle, sender=None):
        ...
        if len(joke_pickle) == 0:
            logging.debug(
                "JokeMachineSession.submit_cb()"
                " -> empty joke_pickle - doing nothing")
            return
        ...
        if jokebook is None:
            logging.error(
                'Joke was submitted to non-existent jokebook id %d',
                jokebook_id)
            return

        # 3. add it to submissions in the appropriate jokebook
        logging.debug(
            "%s submitted a joke to my jokebook#"
            " %d with text: %s and answer %s",
            joke.joker,
            jokebook_id,
            joke.text,
            joke.answer)
        ...
        # 4. alert the owner
        message = str(joke.joker) + \
            _(' submitted a joke to ') + str(jokebook.title)
    ...
    def broadcast_joke_cb(self, jokebook_id, joke_pickle, sender_nick, sender):
        ...
        if jokebook is None:
            logging.error(
                'Joke was broadcast to non-existent jokebook id %d',
                jokebook_id)
            return

        # 3. add it to jokes in the appropriate jokebook
        logging.debug(
            "%s broadcast a joke to my jokebook# %d"
            " with text: %s and answer %s",
            joke.joker,
            jokebook_id,
            joke.text,
            joke.answer)
