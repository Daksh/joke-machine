...
def PersistentProperty(function):
    ...
    def fsetter(obj, name, setter, value):
        setattr(
            obj,
            '__dirty__',
            True)  # TODO -> Propogate dirty flag up to any
        #         parent objects ?
    ...
    def introspect(frame, event, arg):
        if event == 'return':
            ...
            if 'set' in locals:
                func_locals['fset'] = lambda obj, value: fsetter(
                    obj, function.__name__, locals['set'], value)
            if 'default' in locals:
                get_function = lambda obj: fgetter(
                    obj,
                    function.__name__,
                    locals['get'],
                    locals['default'])
                ...
                get_function = lambda obj: fgetter(
                    ...
                    locals['get'],
                    lambda x: None)
            # tag the getter so we can id the
            get_function.__decorator__ = PersistentProperty
