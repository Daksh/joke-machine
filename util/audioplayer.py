...
class AudioPlayer(object):
    ...
    def uri():
        ...
        def set(self, value):
            ...
            self.pipeline.get_by_name('source').set_property(
                'location',
                self.__uri)
    ...
    def raw():
        ...
        def set(self, value):
            ...
            self.uri = name
            logging.debug(
                'AudioPlayer - set_raw wrote %d bytes to %s',
                len(value),
                name)
    ...
    def __build_pipeline(self):
        ...
        decoder.connect(
            "new-decoded-pad",
            self.__new_decoded_pad)  # , converter)
    ...
    def __new_decoded_pad(self, dbin, pad, islast):  # , converter)
        converter = self.pipeline.get_by_name(
            'converter')  # TODO - pass by arg
