...
class JokeViewer(Page):
    ...
    def __init__(self, joke, jokebook_title=''):
        ...
        self.left.pack_start(
            Gtk.Label(_('Joke') + ' ' + str(joke.id)), False, False, 0)
        self.left.pack_start(
            Gtk.Label(_('By') + ' ' + str(joke.joker)), False, False, 0)
...
class Joke(Page):
    ...
    def __do_clicked_answer(self, button, jokebook, joke_id):
        ...
        player = AudioPlayer()
        if jokebook.sound_blob is not None:
        ...
        if len(jokebook.jokes) > joke_id + 1:
            ...
            button.connect(
                'clicked',
                self.__do_clicked_next,
                jokebook,
                joke_id + 1)
            self.navigation_box.pack_start(button, False, False, 0)

        # only allow submitting a joke if activity is shared and you are the
        # one joining
        if not Globals.JokeMachineActivity.is_initiator:
            button = Gtk.Button(_('Submit a Joke'))
            button.connect(
    ...
    def __do_clicked_submit(self, button, jokebook, joke_id):
        Globals.JokeMachineActivity.set_page(
            pages.submit.Submit,
            jokebook,
            joke_id)
