...
# deprecated ?! Gone from build >542 ? Ke ?
...
class JokeMachineActivity(activity.Activity):
    ...
    def __setup(self):
        ...
        for channel_path in channel_paths:
            ...
            if htype == telepathy.HANDLE_TYPE_ROOM:
                logging.debug(
                    'Found our room: it has handle# %d %s',
                    handle,
                    self.__telepathy_connection.InspectHandles(
                        htype,
                        [handle])[0])
                room = handle
    ...
    def __do_activity_shared(self, activity):
        ...
        id = self.tubes_chan[
            telepathy.CHANNEL_TYPE_TUBES].OfferDBusTube(
            MESH_SERVICE,
            {})
    ...
    def read_file(self, file_path):
        ...
        logging.debug(
            'Reading file from datastore via Journal: %s' %
            file_path)
        f = open(file_path, 'r')
        pickle = f.read()
        if len(pickle) == 0:
    ...
    def write_file(self, file_path):
        ...
        if len(Globals.JokeMachineState.jokebooks) != 0:
            logging.debug(
                'Writing file to datastore via Journal: %s' %
                file_path)
            f = open(file_path, 'w')
