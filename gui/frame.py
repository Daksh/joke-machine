...
class Frame(Gtk.Box):
    ...
    def page_class(self):
        if self.__page_class is None:
            # say, for e.g. we're connecting to another activity
            # and we haven't set a default page yet
    ...
    def page():
        ...
        def set(self, value):
            ...
            if not Globals.JokeMachineActivity.is_initiator \
               and isinstance(value, pages.choose.Choose):
                ...
                self.__button_make.set_visible(False)
            elif isinstance(value, pages.choose.Choose):
                self.__button_read.set_visible(False)
                self.__button_make.set_visible(True)
            elif isinstance(value, pages.edit.Edit):
                self.__button_read.set_visible(True)
                self.__button_make.set_visible(False)
            elif isinstance(value, pages.preview.Preview):
                self.__button_read.set_visible(True)
                self.__button_make.set_visible(False)
            else:
                self.__button_read.set_visible(True)
